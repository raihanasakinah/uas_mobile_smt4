...
import 'package:uas_mobile/screen_page/page_edit_mahasiswa.dart';
import 'package:uas_mobile/screen_page/page_insert_mahasiswa.dart';
  ...
  Future<void> getMahasiswa() async {
    try {
      var response = await http
          .get(Uri.parse('http://192.168.100.128/uas_mobile/listmahasiswa.php'));
      if (response.statusCode == 200) {
        var jsonData = json.decode(response.body);
        if (jsonData['isSuccess'] == true) {
          List<Datum> mahasiswas = (jsonData['data'] as List)
          ...
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
              content:
                  Text('Failed to load mahasiswa: ${jsonData['message']}')));
        }
      } else {
  ...
  Future<void> deleteMahasiswa(String id) async {
    try {
      var response = await http.post(
        Uri.parse('http://192.168.100.128/uas_mobile/deletemahasiswa.php'),
        body: {'id': id},
      );
      var jsonData = json.decode(response.body);
      if (response.statusCode == 200 && jsonData['is_success'] == true) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Mahasiswa deleted successfully')));
      } else {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content:
                Text('Failed to delete mahasiswa: ${jsonData['message']}')));
      }
    } catch (e) {
                                        ...
                                        onPressed: () {
                                          Navigator.push(
                                            ...
                                            if (updatedMahasiswa != null) {
                                              setState(() {
                                                int index = mahasiswaList!
                                                    .indexWhere((mahasiswa) =>
                                                        mahasiswa.id ==
                                                        updatedMahasiswa.id);
                                                if (index != -1) {
                                                  mahasiswaList![index] =
